...
from summarise_utils import summarise_file  # <-- add this import
...
if __name__ == "__main__":
    ...
    if '--create-project' in sys.argv:
        ...
        if len(sys.argv) > idx + 1:
            desc = sys.argv[idx + 1]
            build_project(desc, model=model)
        else:
        ...
        if len(sys.argv) > idx + 2:
            ...
            summarise_file(prompt, file_path, model="mistral:7b")
        else:
        ...
        if explain_flag is not None:
            explain_last_n_commands_with_output(explain_flag, model=model)
        else:
            print("Usage:")
